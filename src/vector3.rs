use crate::{J2000, Matrix3, MatrixError, PosVel, Vector3};
...
pub fn normalize<T: RealField>(vec: &Vector3<T>) -> Result<Vector3<T>, MatrixError> {
...
pub fn try_j2000_vel_from_lvlh<T: RealField>(
    lvlh_vel: &Vector3<T>,
    ref_state: &J2000<T>,
) -> Result<Vector3<T>, MatrixError> {
    let J2000(PosVel { pos: rou_ref, vel }) = ref_state;
    let rou_hat = normalize(&ref_state.pos)?;
    let h_hat = normalize(&rou_ref.cross(vel))?;
    let a = Matrix3::from_columns(&[rou_hat.clone(), h_hat.cross(&rou_hat), h_hat]).transpose();
    Ok(a.try_inverse().ok_or(MatrixError::InverseSingular)? * lvlh_vel)
}

pub fn try_j2000_vel_from_vvlh<T: RealField>(
    vvlh_vel: &Vector3<T>,
    ref_state: &J2000<T>,
) -> Result<Vector3<T>, MatrixError> {
